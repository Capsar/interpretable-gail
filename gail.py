from rl_helper import QLearning, DecisionTree, DiscriminatorNN
import numpy as np
from scipy.spatial import distance

def do_gail(expert: QLearning, generator: DecisionTree, discriminator: DiscriminatorNN, n_e_trajectories=(20,5), epochs=10, ownGeneratorTrajectories=False, hasAccessToExpert=False, sampleWithQ=False, discriminateWithQ=True, max_sample_size = 1000):
    ...
    for i in range(epochs):
        ...
        if sampleWithQ:
            ...
            idx = np.random.choice(len(sample_state_actions), size=min(len(sample_state_actions), max_sample_size), p=sample_qs)
