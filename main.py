...
if doGAIL:
    ...
    total_tests = 500
    e_mean, e_std = expert.get_average_reward(total_tests, print=False)
                ...
                for ownGeneratorTrajectories in [False, True]:
                    for hasAccessToExpert in [False, True]:
                        for sampleWithQ in [False, True]:
                            for discriminatorWithQ in [False, True]:
                                ...
                                discriminator = DiscriminatorNN(env, hidden_dims=[32,32], epochs=500, discriminateWithQ=discriminatorWithQ)
                                generator, gail_expert_trajectories = do_gail(expert, generator, discriminator, n_e_trajectories=n_e_trajectories, epochs=epochs, 
                                                                        ownGeneratorTrajectories=ownGeneratorTrajectories, hasAccessToExpert=hasAccessToExpert, sampleWithQ=sampleWithQ, discriminateWithQ=discriminatorWithQ, pprint=False)
                                ...
                                file_name_extra = f'{env.spec.id}_{depth}_{n_e_trajectories[0]}_{n_e_trajectories[1]}_{epochs}_{ownGeneratorTrajectories}_{hasAccessToExpert}_{sampleWithQ}_{discriminatorWithQ}_{len(gail_expert_trajectories)}_{str(round(e_mean, 2))}_{str(round(e_std, 2))}_{str(round(b_mean, 2))}_{str(round(b_std, 2))}_{str(round(g_mean, 2))}_{str(round(g_std, 2))}'
                                ...
                                print(env.spec.id, depth, n_e_trajectories[0], n_e_trajectories[1], epochs, ownGeneratorTrajectories, hasAccessToExpert, sampleWithQ, discriminatorWithQ, len(gail_expert_trajectories), e_mean, e_std, b_mean, b_std, g_mean, g_std)
